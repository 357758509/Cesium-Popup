...
;(function ($, window, document, undefined) {
	...
	Tree.prototype.subscribeEvents = function () {
		...
		if(typeof (this.options.onNodeRemove) === 'function') {
			this.$element.on('nodeRemove',this.options.onNodeRemove);
		}
	...
	*/
	...
	Tree.prototype.buildTree = function (nodes, level) {
		...
		$.each(nodes, function addNodes(id, node) {
			...
			var treeItem = $(_this.template.item)
				...
				.addClass(node.state.checked ? 'node-checked' : 'node-unchecked')
			...
			if (_this.options.showCheckbox) {
                ...
                treeItem.append($(_this.template.chk));
			}
    ...
    Tree.prototype.addNode = function (parentNode, childNode) {
        if(!parentNode.nodes){
            parentNode.nodes = [];
        }
    ...
    Tree.prototype.removeNode2 = function (node) {
        ...
        if(pId){
            ...
            if(pNode){
                ...
                while(i--){
                    if(pNode.nodes[i].nodeId === node.nodeId){
                        index = i;
                        break;
                    }
                }
                if(index !== -1){
                    pNode.nodes.splice(index,1);
                    this.setInitialStates({ nodes: this.tree }, 0);
                    this.render();
                }
            }
    ...
    Tree.prototype.removeNode = function (node) {
        ...
        if(pId !== undefined){
            ...
            if(pNode){
                ...
                if(index !== -1){
                    pNode.nodes.splice(index,1);
                    if(pNode.nodes.length == 0 && pNode.level != 1){
	...
	Tree.prototype.template = {
        ...
        chk : '<div class="squaredTwo"><input type="checkbox" checked=""><label class="check-icon"></label></div>',
        del : '<span class="fui-cross"></span>'
	};
