...
let viewer;
let isShowToolbar = true;
let selectVal = 1;
let isCallInMoving = false;

onMounted(() => {
    (async ()=>{
        proxy.tooltip = createTooltip(document.body);

        viewer = await AMap.init('cesiumContainer');
        ...
        debugger;
        proxy.isShowToolbar = true;
        $('#loadingbar').remove();
        // pure object used as a set.
        proxy.selectedObjs = Object.create(null, {});
        proxy.resetIds = Object.create(null, {});
        let that = proxy;
        ...
        try {
            ...
            let promise1 = scene.open(URL_CONFIG.SCENE_CBD);
